...
## MAKE SURE AND SET THESE VARIABLES PRIOR TO RUNNING
analysisName=2014-09-30_genesOnly
intervals2include=Pf3D7_v9.3_genesMinusMasks.bed
hiPoolName=HIGHPL_CTTGT_L001.realn.bam
loPoolName=LOWPL_GCCAA_L001.realn.bam

## GENERATE MPILEUP
samtools mpileup -B -q 10 -f /proj/julianog/refs/Pf3D7_v9.3/PlasmoDB-9.3_Pfalciparum3D7_Genome.fasta aln_bwa/$hiPoolName aln_bwa/$loPoolName -l intervals/$intervals2include > poo2/$analysisName.mpileup

## CONVERT MPILEUP TO SYNC FORMAT
java -jar ~/bin/popoolation2_1201/mpileup2sync.jar --input poo2/$analysisName.mpileup --output poo2/$analysisName.sync --fastq-type sanger --min-qual 20 --threads 8

## SUBSAMPLING
perl ~/bin/popoolation2_1201/subsample-synchronized.pl --input poo2/$analysisName.sync --output poo2/$analysisName_subsampled.sync --target-coverage 40 --max-coverage 250 --method withreplace
...
perl ~/bin/popoolation2_1201/fisher-test.pl --input poo2/$analysisName_subsampled.sync --output poo2/$analysisName_snp.fet --min-count 2 --min-coverage 4 --max-coverage 250 --suppress-noninformative

## CONVERT SNPWISE FET TO IGV FORMAT
perl ~/bin/popoolation2_1201/export/pwc2igv.pl --input poo2/$analysisName_snp.fet --output poo2/$analysisName_snp_fet.igv

## CALCULATE SNPWISE FST
perl ~/bin/popoolation2_1201/fst-sliding.pl --window-size 1 --step-size 1 --suppress-noninformative --input poo2/$analysisName_subsampled.sync --min-covered-fraction 1.0 --min-coverage 15 --max-coverage 40 --min-count 3 --output poo2/$analysisName_snp.fst --pool-size 40

## CONVERT SNPWISE FST FILE TO IGV FORMAT
perl ~/bin/popoolation2_1201/export/pwc2igv.pl --input poo2/$analysisName_snp.fst --output poo2/$analysisName_snp_fst.igv

## CALCULATE FST BY WINDOW
perl ~/bin/popoolation2_1201/fst-sliding.pl --window-size 1000 --step-size 500 --suppress-noninformative --input poo2/$analysisName_subsampled.sync --min-covered-fraction 1.0 --min-coverage 15 --max-coverage 40 --min-count 3 --output poo2/$analysisName_snp.fst --pool-size 40
...
#perl ~/bin/popoolation2_1201/create-genewise-sync.pl --input poo2/$analysisName_subsampled.sync --gtf /proj/julianog/refs/Pf3D7_v9.3/PlasmoDB-9.3_Pfalciparum3D7.gtf --output poo2/$analysisName_gene.sync

## CALCULATE GENEWISE FISHER'S EXACT
#perl ~/bin/popoolation2_1201/fisher-test.pl --input poo2/$analysisName_gene.sync --output poo2/$analysisName_gene.fet --min-count 2 --min-coverage 4 --max-coverage 250 --suppress-noninformative

## CONVERT GENEWISE FET TO IGV FORMAT
#perl ~/bin/popoolation2_1201/export/pwc2igv.pl --input poo2/$analysisName_gene.fet --output poo2/$analysisName_gene_fet.igv

## CALCULATE GENEWISE FST
#perl ~/bin/popoolation2_1201/fst-sliding.pl --window-size 1 --step-size 1 --suppress-noninformative --input poo2/$analysisName_gene.sync --min-covered-fraction 1.0 --min-coverage 15 --max-coverage 40 --min-count 3 --output poo2/$analysisName_gene.fst --pool-size 40

## CONVERT GENEWISE FST FILE TO IGV FORMAT
#perl ~/bin/popoolation2_1201/export/pwc2igv.pl --input poo2/$analysisName_gene.fst --output poo2/$analysisName_gene_fst.igv
